...
import { PlusOutlined, SearchOutlined, VerticalAlignBottomOutlined, WarningOutlined, PrinterOutlined, FileImageOutlined, QrcodeOutlined } from '@ant-design/icons';
import PrintBarcode from '../inputInventory/printBarcode';
...
const PrintQRCode: React.FC = () => {
    ...
    const [dataReceiptVmImei, setDataReceitImeiAPI] = useState((() => {
    ...
    const UpdateInTem = async (_dataNXNL : any, dataReceiptVmImei: any) => {
        const data = {
            receiptID: _dataNXNL.idnx,
            congviec: businessID,
            imeis: dataReceiptVmImei.map((p:receiptImei) => p.receiptImeiID)
        };
        await receiptService.UpdateWorkProcess(data);
        // await GetListNXNL(modelRequest, function (datas: Array<receiptVm>) {
        //     var nxhhItem = datas.find((element) => {
        //         return element.receiptID === _dataNXNL.receiptID;
        //     });
        //     if (nxhhItem) {
    ...
    const AddReceiptDetailAndImei = (e: any) => {
        ...
        if (dataInputReceiptDetailVm.receiptDetailID <= 0) {
            var chitietNhapXuat = [...dataReceiptVmImei];
            for (var i = (dataReceiptVmImei.length + 1); i <= dataInputReceiptDetailVm.quantity; i++) {
    ...
    const handleKeyDownInput_Sokg = async (e: any, index: number) => {
        if (e.key === 'Enter') {
            ...
            const _dataReceiptVmImei = [...dataReceiptVmImei];
            _dataReceiptVmImei[index].weight1 = weight;
            setDataReceitImeiAPI([]);
            setTimeout(() => {
                rf_datagrid_imei_ctnx.current.cancelEdit();
                setDataReceitImeiAPI(_dataReceiptVmImei);
                setDataInputReceiptImei({ ...dataInputReceiptImei, weight1: value });
                rf_datagrid_imei_ctnx.current.scrollTo(_dataReceiptVmImei[index]);
                setSpinningLoadCTBang(false);
            }, 400);
        }
    }
    ...
    const handleOnBlurInput_Sokg = (e: any, index: number) => {
        ...
        var weight = value.replace(/,/g, '');
        const _dataReceiptVmImei = [...dataReceiptVmImei];
        _dataReceiptVmImei[index].weight = weight;
        setDataReceitImeiAPI([]);
        setTimeout(() => {
            rf_datagrid_imei_ctnx.current.cancelEdit();
            setDataReceitImeiAPI(_dataReceiptVmImei);
    ...
    const onSelectKho_Inrow_CTNXHH = async (value: any, option: any, index: number) => {
        setSpinningLoadCTBang(true);
        const _dataReceiptVmImei = [...dataReceiptVmImei];
        _dataReceiptVmImei[index].width = value;
        setDataReceitImeiAPI([]);
        setTimeout(() => {
            rf_datagrid_imei_ctnx.current.cancelEdit();
            setDataReceitImeiAPI(_dataReceiptVmImei);
    ...
    const onSelectDay_Inrow_CTNXHH = async (value: any, option: any, index: number) => {
        setSpinningLoadCTBang(true);
        const _dataReceiptVmImei = [...dataReceiptVmImei];
        _dataReceiptVmImei[index].thickness = value;
        setDataReceitImeiAPI([]);
        setTimeout(() => {
            rf_datagrid_imei_ctnx.current.cancelEdit();
            setDataReceitImeiAPI(_dataReceiptVmImei);
    ...
    const saveReceipt = async (e: any) => {
        ...
        let totalWeight1 = 0;
        for (var i = 0; i < dataReceiptVmImei.length; i++) {
            totalWeight1 += dataReceiptVmImei[i].weight1;
        }
        ...
        var data = await receiptService.UpdateReceipt(dataReceiptVm, dataLstReceiptDetail, dataReceiptVmImei);
    ...
    const showFormUploadFile = (sortOrder: number) => {
        ...
        if (dataReceiptVmImei.length > 0) {
            var dataHangHoang = dataReceiptVmImei[sortOrder];
            var listFile = [] as any;
    ...
    const handleUplaod = async (option: object) => {
        ...
        if (data.status === APIStatus.ERROR) {
            ...
            var dataHangHoang = dataReceiptVmImei[showUploadRowIndex];
            var _fileUpload = [...fileUpload];
                        ...
                        <div className="pannel-right-body">
                            ...
                            <div className="fl-lef w-25">
                                ...
                                <div className="inline-bolck input-control">
                                    <Input size="small" disabled={!optionReceipt.isEditing || (optionReceipt.isEditing && dataReceiptVm.receiptID > 0)} readOnly name="receiptNo" value={dataReceiptVm.receiptNo}/>
                                </div>
                            </div>
                            <div className="fl-lef w-25">
                                <div className="lable-cotrol inline-bolck mr-right-5">
                                ...
                                <div className="inline-bolck input-control">
                                    <Input size="small" disabled={!optionReceipt.isEditing} name="licensePlate" value={dataReceiptVm.licensePlate} />
                                </div>
                            </div>
                            <div className="fl-lef w-50">
                                <div className="lable-cotrol inline-bolck mr-right-5">
                                ...
                                <div className="code-box-description">
                                    <div className="tool-bar-cthh" style={{ marginBottom: 2 }}>
                                        ...
                                        <div className="inline-bolck w-6 mr-1">
                                            <div className={true ? "ant-input-number-sm ant-input-number" : "ant-input-number-sm ant-input-number ant-input-number-disabled"}>
                                                <NumericFormat
                                                    readOnly={!true}
                                        ...
                                        <div className="inline-bolck w-3 mr-1 text-center">
                                            <Button size="small" disabled={true} type="primary" icon={<VerticalAlignBottomOutlined />}
                                        ...
                                        <div className="inline-bolck w-8 mr-1">
                                            <div className={true || (dataReceiptVmImei != null && dataReceiptVmImei.length <= 0) ? "ant-input-number-sm ant-input-number" : "ant-input-number-sm ant-input-number ant-input-number-disabled"}>
                                                <NumericFormat
                                                    readOnly={!true}
                                        ...
                                        <div className='inline-bolck w-25 mr-1 text-center'>
                                            <Button className='w-100' size="small" disabled={!optionReceipt.isEditing || dataReceiptVmImei != null && dataReceiptVmImei.length <= 0} type="primary" icon={<WarningOutlined />}
                                    ...
                                    <div className="hanghoaInputNXNL">
                                        <Spin spinning={spinningLoadCTBang}>
                                            <DataGrid
                                                ...
                                                editMode="cell"
                                                showFooter
                                                footerData={getFooterData(dataReceiptVmImei)}
                                            >
                        ...
                        <div className="pannel-right-footer">
                            {
                                dataReceiptVm.receiptID > 0 ?
                                    <div className="inline-bolck">
                                        <ReactToPrint
                                            onAfterPrint={() => UpdateInTem(dataReceiptVm, dataReceiptVmImei)}
                                            trigger={() => <Button className="button" disabled={optionReceipt.isSubmit} type="primary" icon={<PrinterOutlined />} size="small">In Mã Vạch</Button>}
                                            content={() => refBarcodePrint.current ? refBarcodePrint.current : eleEmty}
                                            pageStyle="@page { size: 10cm 3cm; margin: 0; padding:0; } @media print { body { -webkit-print-color-adjust: } .barcodePage { page-break-after: always; page-break-inside: avoid; } }"
                                        />
                                        <Button className="button" type="primary" icon={<QrcodeOutlined />} size="small" onClick={e => setShowModalBarcode(true)}>Xem Mã Vạch</Button>
                                    </div>
                                    :
                                    <div className="inline-bolck"></div>
                            }
                            <div style={{ display: "none" }}>
                                <div ref={refBarcodePrint}>
                                    <PrintBarcode receiptVm={dataReceiptVm} receiptDetailVm={(dataLstReceiptDetail.length > 0 ? dataLstReceiptDetail[0] : dataInputReceiptDetailVm)} dataLstIMEI={dataReceiptVmImei} branchInfo={branchInfo} />
                                </div>
                            </div>
                            <Modal
                                title="Xem Barcode"
                                visible={showModalBarcode}
                                onOk={() => setShowModalBarcode(false)}
                                onCancel={() => setShowModalBarcode(false)}
                                style={{ width: 900 }}
                            >
                                <PrintBarcode receiptVm={dataReceiptVm} receiptDetailVm={(dataLstReceiptDetail.length > 0 ? dataLstReceiptDetail[0] : dataInputReceiptDetailVm)} dataLstIMEI={dataReceiptVmImei} branchInfo={branchInfo} />
                            </Modal>
